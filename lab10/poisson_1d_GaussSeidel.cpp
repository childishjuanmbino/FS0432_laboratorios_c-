    ...
    while (iter < max_iter && error > tol)
{
    ...
    for (int i = 1; i < N; i++)
    {
        ...
        u[i] = 0.5 * (u[i - 1]+ u[i + 1]- h * h * fuente(x[i]));

        error = std::max(error, std::abs(u[i] - valor_anterior));
    }

    // Error respecto a la solucion exacta
...
<<<<<<< Updated upstream
plt::plot(x, u, {{"label","GaussSeidel"}});
...
plt::title("Convergencia del metodo de GaussSeidel");
